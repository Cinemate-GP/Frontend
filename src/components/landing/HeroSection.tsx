    ...
    <div className="relative h-screen flex items-center overflow-hidden z-10">
      ...
      >
        <Image
          ...
          width={64}
          height={64}
          className="w-10 h-10 sm:w-16 sm:h-16 lg:w-12 lg:h-12 object-contain drop-shadow-lg hover:scale-105 transition-transform duration-300"
          priority
        />
        <span className="text-xl sm:text-4xl lg:text-4xl font-bold tracking-tight drop-shadow-lg select-none">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="hidden sm:block absolute left-1/2 top-0 h-full w-[55%] overflow-visible pointer-events-none z-10">
          <div
            className="relative w-full h-[80vh] flex flex-row items-center justify-center gap-4 lg:gap-8"
            style={{ top: "8vh" }}
          >
            {posterArt.length > 0 &&
              [0, 1, 2].map((col) => {
                // Offset posters for each column
                const posters = Array.from({ length: postersPerCol }).map(
                  (_, i) => {
                    ...
                    {postersLoop.map((p, i) => (
                      ...
                      >
                        <Image
                          ...
                          alt="Trending Movie Poster"
                          fill={true}
                          className="object-cover rounded-xl shadow-xl group-hover:brightness-110 group-hover:shadow-2xl group-hover:ring-4 group-hover:ring-primary
