...
export default function Sidenav() {
        ...
        return links.map((link) => {
            ...
            return (
                <li key={link.name} className="relative group">
                    ...
                    >
                        <span className={`${isCollapsed ? "" : "mr-3"} transition-all duration-200`}>
                          {icons[iconName] ?
    ...
    return (
        <>
            <div
                className={`h-screen text-gray-300 bg-black border-r fixed z-50 border-gray-600 transition-all duration-300 flex flex-col ${
                    isCollapsed ? "w-16" : "w-45"
                } hidden md:flex`}
            >
                <div className="flex items-center justify-between px-4 py-5 border-b border-gray-600">
                    <Link href={"/pages"} className={`flex items-center ${isCollapsed ? "justify-center w-full" : ""}`}>
                        <Image
                            src="/logo.png"
                        ...
                        />
                        {!isCollapsed && <span className="ml-2 font-bold text-lg">Cine<span className="text-red-600">Mate</span></span>}
                    </Link>

                    <button
                        onClick={toggleSidebar}
                        className={`text-gray-500 hover:text-gray-300 focus:outline-none transition-all duration-200 ${isCollapsed ? "hidden" : "block"}`}
                    >
                        {<span>←</span>}
                    </button>
                ...
                <div className={`mt-auto border-t border-gray-600 p-3 ${isCollapsed ? "flex justify-center" : ""}`}>
                    <div className={`flex ${isCollapsed ? "flex-col" : "items-center justify-between"} gap-2`}>
                        ...
                        >
                            <div className="relative">
                                {icons["Profile"] ?
                                    React.createElement(icons["Profile"], { className: "w-6 h-6 text-gray-400" }) :
                        ...
                        >
                            {icons["Settings"] ?
                                React.createElement(icons["Settings"], { className: "w-6 h-6 text-gray-400" }) :
                                <div className="w-6 h-6 flex items-center justify-center text-gray-400">⚙️</div>
                            }
